...
namespace ElectronicObserver.Window {
	...
	public partial class FormQuest : DockContent {
		...
		void Updated() {
			...
			foreach ( var q in KCDatabase.Instance.Quest.Quests.Values ) {
...

				{
					...
					if ( q.State == 3 ) {
                        value = LoadResources.getter("FormQuest_1");
						tag = 1.0;
			...
			if ( KCDatabase.Instance.Quest.Quests.Count != KCDatabase.Instance.Quest.Count ) {
				...
				QuestView.Rows[index].Cells[QuestView_Name.Index].Value = string.Format( LoadResources.getter("FormQuest_2"), ( KCDatabase.Instance.Quest.Count - KCDatabase.Instance.Quest.Quests.Count ) );
			}

			if ( KCDatabase.Instance.Quest.Quests.Count == 0 ) {
				int index = QuestView.Rows.Add();
				QuestView.Rows[index].Cells[QuestView_State.Index].Value = null;
                QuestView.Rows[index].Cells[QuestView_Name.Index].Value = LoadResources.getter("FormQuest_3");
			}
		...
		private void MenuMain_Initialize_Click( object sender, EventArgs e ) {
            ...
            if (MessageBox.Show(LoadResources.getter("FormQuest_4"), LoadResources.getter("FormQuest_5"),
		...
		private void ClearQuestView() {
...

			{
				DataGridViewRow row = new DataGridViewRow();
				row.CreateCells( QuestView );
                row.SetValues(null, null, null, LoadResources.getter("FormQuest_6"), null);
				QuestView.Rows.Add( row );
			}
