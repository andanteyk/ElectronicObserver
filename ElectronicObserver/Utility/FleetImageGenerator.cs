		...
		public static Bitmap GenerateTestBitmap( FleetImageArgument args ) {
			...
			using ( var g = Graphics.FromImage( bitmap ) ) {
				...
				for ( int fleetIndex = 0; fleetIndex < args.FleetIDs.Length; fleetIndex++ ) {
					...
					{	// fighter power
						var iconpos = fleetPointer + GetAlignmentOffset( ContentAlignment.MiddleLeft, EquipmentIconSize, parameterAreaUnitSize );
					...
					{	// searching ability
						...
						g.DrawString( fleet.GetSearchingAbilityString(), args.MediumDigitFont, mainTextBrush, new Rectangle( fleetPointer + GetAlignmentOffset( ContentAlignment.MiddleLeft, paramValueSize, parameterAreaUnitSize ), paramValueSize ), formatMiddleLeft );
						fleetPointer.X += paramValueSize.Width + fleetParameterAreaInnerMargin.Width;
					}

					fleetPointer.X = fleetPointerOrigin.X;
					fleetPointer.Y += parameterAreaUnitSize.Height;
