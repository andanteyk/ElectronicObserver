...
* 「泊地修理タイマ」は各プレイヤーに1つだけ存在する。これは修理を開始した時にその時刻がセットされるものである(要するに `DateTime` 型の変数だと思えばよい)。  
    * 初期値を `null` とする。この値は修理が行われていないことを示す。(これは説明の便宜上のもので、実際に `null` であるかは知らない。)  
* 泊地修理タイマに現在の時刻をセットすることを「スタート」とする。スタートしたとき、以前に設定されていた時刻は上書きされる。  
また、タイマに `null` をセットすることを「ストップ」とする。
* 「修理可能な艦」は、入渠しておらず、遠征中ではない、 `50% < HP < 100%` である艦のことである。

「通常の編成を行った結果、旗艦が工作艦であるとき」に、タイマはスタートする。編成操作に工作艦が直接関わる必要はない。具体的には以下の状態などが挙げられる。

* 旗艦に工作艦を配置した
* 2番艦に工作艦がいるときに1番艦をはずした
* 旗艦に工作艦がいるときに2番艦と3番艦を入れ替えた

なお、「編成」は `api_req_hensei/change` を用いるもの（艦の追加/はずす/入れ替え/D&Dによる操作/旗艦以外全解除）であり、**編成記録/展開による編成や解体・近代化改修による除籍は含まない**。  
...
また、途中で編成を変更したとしても、タイマーが即時ストップすることはない(但し、編成を行えば上記のとおり再スタートされる)。  
そのため、「編成」として扱われない編成記録/展開を利用することで、タイマを再スタート(リセット)させることなく編成の変更を行うことができる。  
...
* 「泊地修理タイマが `null` である場合」もしくは「泊地修理タイマがセットされており、20分経過していない場合」は、なにもしない
* 「泊地修理タイマがセットされており、20分以上経過している場合」は、下記の修理実行判定を行う
    * 判定に成功すれば経過時間に応じてHPを回復し、泊地修理タイマをスタートする
    * 失敗した場合、旗艦が工作艦の艦隊があれば泊地修理タイマをスタートする。なければ泊地修理タイマをストップする
...
* `api_req_hensei/change` で変化した艦隊の旗艦が工作艦なら `timer = Now`
* `api_req_hensei/preset_select` で旗艦が工作艦になり、かつ `timer == null` ならば `timer = Now`
* `api_port/port` で、`( Now - timer ) >= 20min` かつ旗艦が工作艦の艦隊があれば `timer = Now`、なければ `timer = null`
...
試製51cm連装砲は、`api_start2`のデータでは、装備種別が[1,1,3,3]=[砲,主砲,大口径主砲,大口径主砲]となっており、なぜか大口径主砲(II)ではない。  
...
調べたところ、`api_get_member/unsetslot` (未装備の装備IDをカテゴリごとに分類したAPI)において、それぞれ38や93として扱われていることが分かった。  
装備種の判定では「装備可能な装備種IDリストの作成 → それに応じて `unsetslot` から装備IDを抽出」の順で処理されるため、本来指定された装備種IDではなく、 `unsetslot` においてどのカテゴリにあるかで判定される。  
サーバから `unsetslot` が送られてくるときは、上記装備は38/93などに配置されるため、本来の装備種とは異なる扱いを受ける。  

なお、2015/09/06 現在、装備の廃棄処理はクライアント側では「装備データ自身の削除」「未装備リストにあればそのIDを削除」の2つの処理が行われているが、未装備リストから削除する際には `SLOTITEM_SPECIAL_FLAGS` の値を基にして検索・削除される。  
この時、大型電探(II)のIDは93に存在するが、 `SLOTITEM_SPECIAL_FLAGS` では43と指定されているため、リストから削除することができない。そのため装備リストにはアイテムが残るが実データは存在しない状態になり、何らかのエラーが発生すると考えられる。  
実際に行うと、クライアントがクラッシュする(猫る)。廃棄API自体は通るため、サーバから装備は削除される。  
...
2015/10/21オンメンテ（九月作戦褒章配布）で追加された「試製景雲(艦偵型)」(ID: 151)だが、 `unsetslot` では94 ( 艦上偵察機(II) ) に分類されている(はず; 未保有なので推測)。  
しかしこちらは同日現在 `SLOTITEM_SPECIAL_FLAGS` にそもそも値が存在しないため、上記電探同様廃棄時にエラーが発生するものと予想される。
