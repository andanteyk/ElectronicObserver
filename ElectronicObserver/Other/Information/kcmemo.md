...
```
与ダメージ > 第一砲撃戦で後に攻撃したほう
```

もしくはランダムであると推測している。（確証は全くなく、単なる予想である。）  
...
雷撃しか行わない潜水艦隊でもMVPは前後で安定しないため、不可視の第一砲撃戦での行動順がキーとなっているか、もしくはランダムであると推測した。  
また、(雷撃のみとして)「旗艦→6番艦→5番艦→…→2番艦」という優先順ではないか、という説もある。  
...
* 展開時に以下の状態である艦娘はスキップされる。全員がその状態の場合は展開できない。
    * 別の艦隊に所属している。
    * 除籍されている。
* 除籍された艦娘はリストからスキップされる。全員が除籍されたリストは削除される。
