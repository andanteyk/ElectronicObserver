...
```
HP1あたりの回復時間[s] = ( 入渠時間[s] - 30s ) / 被ダメージ
回復HP = max( int( ( 泊地修理の経過時間[s] - 70s ) / HP1あたりの回復時間 ), 1 )
```

なお、回復HPの `-70` は、「全回復時に通常の入渠に比べて +30～+40秒 かかる」ためである。  
この幅については確定情報がないが、ランダムというよりは艦によって決定的に定まるらしい。  

例を挙げると、[「Lv97倍率1の艦種がHP2回復に掛かる時間」の泊地修理における加算は +68秒 程度となる](https://twitter.com/KancolleSniffer/status/864486346462818305)。
...
また、拡張スロットでは 17:機関部強化 が無条件に装備可能になるようになっている。  
>（現在入手可能な艦種はすべて元から装備可能であるため意味は薄い）  
> なお、拡張スロットでは別の処理で「機関部強化の装備の中でも 33:改良型艦本式タービン のみ装備可能とする」とされているため、実質装備できるのはそれだけになる。
...
|--:|:--|:--|:--|
|201|敵艦隊を撃破せよ！|勝利1
|216|敵艦隊主力を撃滅せよ！|戦闘1 ※ボス戦闘での敗北はカウントに含まない
